...
public sealed class ApplyProposalApprovedUseCaseTests
{
    [Fact]
    public async Task ExecuteAsync_WithValidEvent_ShouldApproveProposal()
    {
        ...
        var useCase = new ApplyProposalApprovedUseCase(dataStore);

        await useCase.ExecuteAsync(tenantId, proposal.Id, Guid.NewGuid(), "ProposalApprovedIntegrationEvent.v1", "SIA.WorkflowService", Guid.NewGuid(), CancellationToken.None);
    ...
    public async Task ExecuteAsync_WhenProposalDoesNotExist_ShouldThrowProposalNotFoundException()
    {
        ...
        await Assert.ThrowsAsync<ProposalNotFoundException>(() =>
          useCase.ExecuteAsync(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), "ProposalApprovedIntegrationEvent.v1", "SIA.WorkflowService", Guid.NewGuid(), CancellationToken.None));
    }
    ...
    public async Task ExecuteAsync_WhenEventAlreadyProcessed_ShouldNotApplyDecisionTwice()
    {
        ...
        await useCase.ExecuteAsync(tenantId, proposal.Id, eventId, "ProposalApprovedIntegrationEvent.v1", "SIA.WorkflowService", Guid.NewGuid(), CancellationToken.None);
        await useCase.ExecuteAsync(tenantId, proposal.Id, eventId, "ProposalApprovedIntegrationEvent.v1", "SIA.WorkflowService", Guid.NewGuid(), CancellationToken.None);
