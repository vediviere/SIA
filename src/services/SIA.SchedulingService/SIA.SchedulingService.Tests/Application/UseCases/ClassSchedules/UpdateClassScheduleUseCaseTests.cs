...
public sealed class UpdateClassScheduleUseCaseTests
{
    ...
    public async Task ExecuteAsync_WithValidData_ShouldUpdateClassSchedule()
    {
        ...
        Assert.Equal("MIERCOLES", dataStore.UpdatedSchedule.Day);
        Assert.NotNull(dataStore.UpdatedEvent);
        Assert.Equal(existingSchedule.Id, dataStore.UpdatedEvent.ClassScheduleId);
    ...
    public async Task ExecuteAsync_WhenTenantIdDoesNotMatchSchedule_ShouldThrowClassScheduleNotFoundException()
    {
        ...
        Assert.Equal("MARTES", existingSchedule.Day);
        Assert.Equal(startTime, existingSchedule.StartTime);
        Assert.Equal(endTime, existingSchedule.EndTime);
