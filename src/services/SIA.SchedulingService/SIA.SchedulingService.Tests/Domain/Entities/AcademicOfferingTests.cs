...
public sealed class AcademicOfferingTests
{
  ...
  [Fact]
  public void AssignClassHours_ValidHours_ShouldUpdateClassHours()
  {
    var academicOffering = new AcademicOffering(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), "OFERTADA");

    academicOffering.AssignClassHours(5);

    Assert.Equal(5, academicOffering.ClassHours);
    Assert.NotNull(academicOffering.UpdatedAtUtc);
  }
  ...
  [InlineData(0)]
  [InlineData(-1)]
  public void AssignClassHours_InvalidHours_ShouldThrowArgumentOutOfRangeException(int classHours)
  {
    var academicOffering = new AcademicOffering(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), "OFERTADA");

    Assert.Throws<ArgumentOutOfRangeException>(() => academicOffering.AssignClassHours(classHours));
  }
