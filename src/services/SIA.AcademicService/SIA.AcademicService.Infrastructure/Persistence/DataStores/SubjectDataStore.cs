...
public sealed class SubjectDataStore : ISubjectDataStore
{
    ...
    {
        return _dbContext.Subjects.AnyAsync(subject => subject.TenantId == tenantId && 
                                            subject.Code == code,
                                            cancellationToken);
    }

    public async Task AddSubjectWithOutboxAsync(
        Subject subject,
        SubjectCreatedIntegrationEvent integrationEvent,
