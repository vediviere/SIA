...
public class SoftDeleteServiceComplementaryUseCaseTests
{
    [Fact]
    public async Task ExecuteAsync_WithValidRequest_ShouldSoftDeleteServiceComplementary()
    {
        // Arrange
        ...
        dataStore.Setup(x => x.SoftDeleteServiceComplementaryWithOutboxAsync(
                It.IsAny<ServiceComplementary>(),
                It.IsAny<ServiceComplementaryDeletedIntegrationEvent>(),
                It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);

        var useCase = new SoftDeleteServiceComplementaryUseCase(dataStore.Object);

        // Act
        await useCase.ExecuteAsync(tenantId, serviceComplementaryId, correlationId, CancellationToken.None);

        // Assert
        Assert.False(serviceComplementary.Status);
        Assert.NotNull(serviceComplementary.UpdatedAtUtc);

        dataStore.Verify(
            x => x.SoftDeleteServiceComplementaryWithOutboxAsync(
    ...
    public async Task ExecuteAsync_WithValidRequest_ShouldSaveOutboxEvent()
    {
        ...
        dataStore.Verify(
            x => x.SoftDeleteServiceComplementaryWithOutboxAsync(
                serviceComplementary,
                It.Is<ServiceComplementaryDeletedIntegrationEvent>(integrationEvent =>
